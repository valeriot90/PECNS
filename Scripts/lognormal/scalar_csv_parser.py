...
def scalar_csv_parser(filename, ordinate):
	...
	# We have values for each aircrafts, so we compute the mean of these values
		...
		repetition_matrix = sca_csv_file.iloc[:7].copy()
		model_row = sca_csv_file.iloc[:7].copy()
		...
		for i in range(7, sca_csv_file.shape[0]):
			model_row[6] = sca_csv_file.iloc[i]
	...
	repetition_matrix.columns = ['X', 'mk', 'mt', 'switchTime', 'wk', 'wt',  ordinate]
