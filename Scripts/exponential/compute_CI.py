...
def compute_CI_from_files(dir_path, ordinate, confidence_level):
...
def compute_CI_from_array(samples, confidence_level):
	num_samples = len(samples)
	square_root_num_samples = math.sqrt(num_samples)
	sample_variance = np.var(samples)
	standard_deviation = math.sqrt(sample_variance)
...
# Main for testing
#if len(sys.argv) == 4:	
#	print(compute_CI_from_files(sys.argv[1], sys.argv[2], sys.argv[3]))
#else:
