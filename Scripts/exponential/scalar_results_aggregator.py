...
def scalar_results_aggregator(dir_path, *args):
	...
	if last_parameter == "responseTime" or last_parameter == "packetReceived" or last_parameter == "numSwitch" or last_parameter == "queueLength":	
		...
		#orders results, 
		results.sort_values(['X','k', 'switchTime','t', last_parameter],inplace = True)
		...
		CI_average_matrix = np.zeros((number_of_scenarios, 6))
		...
		for i in range (0, int(number_of_scenarios)):  #number of scenarios
			CI_average_matrix[i,:4] = np_results[i*repetitions, :4]
			CI_average_matrix[i, 4] = np.mean(np_results[i*repetitions:(i+1)*repetitions,4])
			CI_average_matrix[i, 5] = compute_CI.compute_CI_from_array(np_results[i*repetitions:(i+1)*repetitions,4], 99)
